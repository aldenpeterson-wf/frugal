...
async def main():
    ...
    process = Popen(['lsof', '-i:55000-55200'], stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
    print(''.join([e for e in stdout.decode('utf-8').split('\n')]), stderr)
    ...
    if args.transport_type in ["stateless", "stateless-stateful"]:
        ...
        try:
            ...
            process = Popen(['lsof', '-i:4222'], stdout=PIPE, stderr=PIPE)
            stdout, stderr = process.communicate()
            print(''.join([e for e in stdout.decode('utf-8').split('\n')]), stderr)
            # print(stdout, stderr)
            raise e
